...
"""E2E smoke test for PR 1's batch-claim surface: get_batch, members, connect,
and detach against claims and a Lease created directly (claim_batch does not
exist yet)."""
...
def _batch_manifest(batch_id: str, warmpool: str) -> str:
    lease_duration = 300
    renew_time = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
    claims = "\n---\n".join(
...
def test_batch_get_batch_members_connect_detach(tc, temp_namespace, sandbox_warmpool):
