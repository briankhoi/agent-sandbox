...
class K8sHelper:
    ...
    def get_sandbox_warmpool(
        ...
        return self._get_claim_group_object(WARMPOOL_PLURAL_NAME, name, namespace, _request_timeout)

    def get_sandbox_template(
        self, name: str, namespace: str, _request_timeout: float | tuple[float, float] | None = None
    ) -> dict[str, Any] | None:
        """Gets a SandboxTemplate custom resource (or ``None`` if it doesn't exist)."""
        return self._get_claim_group_object(TEMPLATE_PLURAL_NAME, name, namespace, _request_timeout)

    def _get_claim_group_object(
        self,
        plural: str,
        name: str,
        namespace: str,
