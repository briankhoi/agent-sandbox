...
class K8sHelper:
    ...
    def read_batch_lease(self, name: str, namespace: str):
        """Reads a batch Lease, or ``None`` if it doesn't exist."""
        try:
            return self.coordination_v1_api.read_namespaced_lease(name, namespace)
        except client.ApiException as e:
            if e.status == 404:
                return None
            raise

    def replace_batch_lease(self, name: str, namespace: str, body):
