...
def _is_transport_error(error: BaseException) -> bool:
    """Whether an error is one the watch loop also treats as transient."""
    if isinstance(error, aiohttp.ClientSSLError):
        return False
    return isinstance(
        error,
        (aiohttp.ClientConnectionError, aiohttp.ClientPayloadError, ConnectionError, asyncio.TimeoutError),
    )
...
class AsyncSandboxBatch:
    ...
    async def _stop_background_tasks(self) -> None:
        """Stops the creators, then cancels the watcher and lease renewal loop."""
        self._stop_creating.set()
        if self._creators:
            # Every create carries a request timeout, so waiting this long means every create
            # that was sent has been answered.
            _, pending = await asyncio.wait(
                self._creators, timeout=batch_utils.BATCH_REQUEST_TIMEOUT_SECONDS + 1
            )
    ...
    def events(self) -> AsyncIterator[BatchEvent]:
        """Returns an async iterator over the batch's events, in the order this handle observed them.
    ...
    def iter_ready_groups(self) -> AsyncIterator[GroupReady]:
        """Returns an async iterator that yields one ``GroupReady`` per group, as soon as its outcome is known.
    ...
    async def _iterate(
        self, pop: Callable[[], _T | None], done: Callable[[], bool]
    ) -> AsyncIterator[_T]:
        while (item := await self._next(pop, done)) is not None:
            yield item

    async def _next(self, pop: Callable[[], _T | None], done: Callable[[], bool]) -> _T | None:
