...
- **Section 2, items 1 and 2 (the proposal and the plan):** read both from `batch-primitives-notes`, not from `batch-claim-proposal` and not by absolute path: `git show batch-primitives-notes:batch-primitives/batch_claim_proposal.md` and `git show batch-primitives-notes:batch-primitives/python_sdk_batch_plan.md`. This prompt is on that branch too, at `batch-primitives/STALE_PR_2_prompt.md`; re-read it there if your context gets compacted.
...
7. **Guard the untracked planning files** so they can never be committed:
   ...
   for f in batch-primitives/python_sdk_batch_plan.md batch-primitives/PR_1_prompt.md batch-primitives/STALE_PR_2_prompt.md; do
